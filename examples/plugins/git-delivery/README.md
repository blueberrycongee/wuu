...
`project-candidate.publish`, which receives `{ candidate, title }` and returns
`{ url }`, and the public runtime invocation contract. Wuu records the returned link
...
`project-candidate.publish`（接收 `{ candidate, title }`，返回 `{ url }`）和公开的运行时
调用契约。Wuu 会把返回的链接记录为这份提案的决定。
