...
func (s *Service) migrate() error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS named_agent_create_requests (
...
func (s *Service) createAgent(ctx context.Context, params CreateNamedAgentParams) (AgentCredential, error) {
	...
	if requestID := strings.TrimSpace(params.RequestID); requestID != "" {
		// Hash the normalized request before choosing a random avatar. Defaults
		// generated by the host must not make a retry look like a new request.
		payload, _ := json.Marshal(CreateNamedAgentParams{
			Name: name, Role: role, AvatarKey: avatarKey, AvatarImage: avatarImage,
			EngineOverride: engine, ProviderOverride: provider, ModelOverride: model,
			EffortOverride: effort, Autostart: params.Autostart,
		})
		digest := sha256.Sum256(payload)
		requestHash := hex.EncodeToString(digest[:])
		// Write before reading so concurrent connections serialize on SQLite's
		// writer lock. The claim and identity are committed together.
		claim, err := tx.ExecContext(ctx, `INSERT INTO named_agent_create_requests(request_id, request_hash, agent_id)
			VALUES (?, ?, ?) ON CONFLICT(request_id) DO NOTHING`, requestID, requestHash, id)
		if err != nil {
			return AgentCredential{}, fmt.Errorf("claim named agent creation request: %w", err)
		}
		...
		if inserted == 0 {
			var storedHash, agentID string
			if err := tx.QueryRowContext(ctx, `SELECT request_hash, agent_id FROM named_agent_create_requests WHERE request_id = ?`, requestID).Scan(&storedHash, &agentID); err != nil {
				return AgentCredential{}, err
			}
			if storedHash != requestHash {
				return AgentCredential{}, fmt.Errorf("%w: named agent creation request was already used with different details", ErrConflict)
			}
			// Release the transaction's connection before the ordinary identity
			// and credential readers use this service's single connection.
			if err := tx.Rollback(); err != nil {
				return AgentCredential{}, err
			}
			agent, err := s.GetNamedAgent(ctx, agentID)
			if errors.Is(err, ErrNotFound) {
				return AgentCredential{}, fmt.Errorf("%w: the previously created named agent was deleted", ErrConflict)
			}
			if err != nil {
				return AgentCredential{}, err
			}
			token, err := s.loadAgentToken(ctx, agentID)
			return AgentCredential{Agent: agent, Token: token}, err
		}
	}
	if avatarKey == "" {
		avatarKey, err = randomNamedAgentAvatarKey()
...
func (s *Service) deleteAgent(ctx context.Context, agent NamedAgent) error {
	...
	for _, removal := range removals {
		...
		ids, err := refreshRoomTurnMembersTx(ctx, tx, removal.roomID, now)
		if err != nil {
			return err
		}
		next = appendUniqueStrings(next, ids...)
...
func (s *Service) UpdateRoom(ctx context.Context, params UpdateRoomParams) (Room, error) {
	...
	next, err := refreshRoomTurnMembersTx(ctx, tx, id, toMillis(s.now()))
	if err != nil {
		return Room{}, err
	}
	if err := tx.Commit(); err != nil {
