...
func requestWakeTx(ctx context.Context, tx *sql.Tx, agentID string, now int64) (bool, error) {
	var outstanding int
	err := tx.QueryRowContext(ctx, `
		SELECT wake.outstanding FROM agent_wake_state wake WHERE wake.agent_id = ? AND (EXISTS (SELECT 1 FROM named_agents agent WHERE agent.id = wake.agent_id AND agent.kind = 'named' AND agent.deleted_at IS NULL) OR EXISTS (SELECT 1 FROM room_runtimes runtime WHERE runtime.id = wake.agent_id AND runtime.autostart = 1))`, agentID,
	).Scan(&outstanding)
	if errors.Is(err, sql.ErrNoRows) {
		return false, fmt.Errorf("%w: named agent %q", ErrNotFound, agentID)
	}
