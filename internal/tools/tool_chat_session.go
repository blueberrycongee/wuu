...
func (t *ChatSessionTool) Definition() providers.ToolDefinition {
	return providers.ToolDefinition{Name: t.Name(), Description: "Discover named agents' continuing conversations and read prior execution results. Each name has one continuing session. Use send for asynchronous peer messages with room_id and request_id; use collaboration_send or chat_task to delegate to another identity. For ordinary project execution use the session tool; chat_session addresses named identities only. List/get include archived execution metadata; prior sessions must not be restarted as parallel copies. Stop pauses a conversation and resume continues it. Private transcripts remain private.", InputSchema: map[string]any{
