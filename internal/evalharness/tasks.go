...
func Catalog() []Task {
	return []Task{
		...
		},
		{
			...
			Prompt:        "Start ./dev.sh with bash run_in_background, use process action=read until READY_FOR_EVAL appears, write observed.txt containing READY_FOR_EVAL, then use process action=stop to stop the process.",
			RequiredTools: []string{"bash", "process"},
