...
ACP engines that advertise models on `session/new` appear in the composer picker. Grok uses that first-class model list (`grok-4.6`, `grok-4.5`, …) plus `session/set_model`; effort uses the agent's `thought_level` option when advertised. If an agent advertises nothing, the composer keeps **Agent default**. Wuu does not substitute its own provider catalog. Programmatic ACP model selection requires an advertised model; OpenCode model IDs use `provider/model`. ACP image attachments are written to local files and included as paths in the prompt so the agent can read them with its own tools. Wuu does not send ACP image content blocks, including when the agent advertises image prompt support. Unsupported host HTTP MCP tools still produce an error rather than dropping the tools.
