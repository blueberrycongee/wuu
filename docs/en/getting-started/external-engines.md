...
Wuu saves the native session reference and reloads it on subsequent turns. If an ACP agent cannot load sessions, or a saved session fails to load, the turn fails without silently creating a replacement history. A turn completes only on the native prompt response, not after a quiet interval. Stop requests cancel native work and clean up the child process. OpenCode uses a password-protected loopback server with a fresh password for each process; it is not exposed as a remote service.
