...
Register model tools in the initialization result's `tools` array. Each tool needs an `id`, `description`, and object `input_schema`. The host creates a namespaced public name. `execution_scopes` can limit availability to `root`, `child`, `collaboration`, or `external`; `activity` describes read-only behavior, concurrency safety, risk, and whether the tool orchestrates child tools. Declare real effects rather than marking a writer read-only to bypass scheduling or permission checks.

An external engine receives only tools that explicitly include `external` in `execution_scopes`; an omitted scope does not opt in. Wuu exposes them through a loopback MCP endpoint bound to the current Wuu session and turn, and supplies the owning plugins' `agent.system_prompt.section` contributions. Tool calls retain the host-assigned session, turn, and call identities and pass through the session permission boundary and plugin authorization. Native engine tools remain under the engine's own permission system. The endpoint closes when the turn ends and resolves the active plugin generation for each call, so a cached tool name cannot invoke a disabled plugin. External MCP results use the tool result's text projection.
