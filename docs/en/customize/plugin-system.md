...
Activation switches the session's plugin bindings under a transaction lock. If candidate activation fails, it is closed. If the associated durable commit fails after rebinding, the old bindings are restored and the candidate is closed. After a successful publication, the old generation is retired; a later cleanup error is reported rather than rolling back the successful replacement.
...
Disabling a plugin removes its active contributions. Storage and settings normally remain, and completed external effects are not undone. Safe mode and default-UI recovery provide ways to regain control after extension failures. Wuu does not audit or certify third-party packages; see the [security model](../reference/security-model.md).
