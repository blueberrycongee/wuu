...
一个补丁内，每个规范化后的路径只能属于一个文件段，移动操作的源路径和目标路径
也计入检查。例如，`a.txt` 和 `./a.txt` 视为同一路径。修改同一文件的多处内容时，
应在一个 `Update File` 段内使用多个 `@@` 块。文件段发生路径冲突会在写入任何文件
或启动 `then_run` 之前校验失败；`dry_run` 也会拒绝相同的冲突。
