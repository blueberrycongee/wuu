...
扩展页沿用设置页的内容列和字体层级，以开放列表代替表单式外框。紧凑的顶部将搜索放在标题和操作附近。插件排在最前，因为它们的运行与授权状态可能需要你做决定；官方技能和你的技能随后。每一行依次是图标、名称及其下方的说明、末尾的状态或所属插件，最后是打开插件详情或技能预览的箭头。长名称换行，保留用于识别的后缀；悬停时整行高亮，键盘焦点沿相同的圆角边界显示。搜索会隐藏所有没有匹配项的分组。内容列较窄时，末尾的标签移到说明下方，说明最多使用两行。

插件详情将图标与标题放在同一行，再展示行内的状态文字、来源和授权范围。权限按能力分组，所有分组共用一列标签。权限以普通文字呈现，不加胶囊外框；自然语言权限使用 UI 字体，并在弹窗内换行。

通过 `/dev/extensions/` 预览，可选参数包括 `theme=dark`、`size=20`、`lang=en`、`long` 和 `empty`。技能与插件包均为示例数据，覆盖每一种状态；详情对话框中的操作只改变预览里的状态。

需要可重复的 macOS Electron 截图时，在 `desktop/` 中启动 `npx vite --config dev/extensions/vite.config.ts`，再运行 `./node_modules/.bin/electron dev/extensions/capture.cjs`。用 `WUU_FIXTURE_ORIGIN` 指定其他端口，用 `WUU_CAPTURE_OUTPUT` 分开保存修改前后的产物。脚本记录截图、渲染边界、图片哈希和运行时版本，覆盖明暗主题、默认与大字号、宽窄窗口、长与空内容、滚动、搜索、悬停、键盘焦点及详情和菜单。权限文字超出弹窗时会报错，这项检查来自已复现的窄窗口缺陷；截图仍需单独目视检查。此独立组件预览不涉及正式 preload、插件运行时或安装。
