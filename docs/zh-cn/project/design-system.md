...
| 角色 | 变量 | 与 UI 基线 U 的关系 | U = 14.5px 时取值 |
| --- | --- | --- | --- |
| 大标题 | `--font-hero` | round(U × 32 / 14) | 33px |
| 展示标题 | `--font-display` | round(U × 24 / 14) | 25px |
| 分区标题 | `--font-heading` | round(U × 20 / 14) | 21px |
| 小节标题 | `--font-title` | round(U × 16 / 14) | 17px |
| 界面与正文 | `--font-ui`、`--font-body`、`--font-content` | U | 14.5px |
| 菜单与紧凑控件 | `--font-menu`、`--font-sm` | round(U × 13 / 14) | 13px |
| 辅助信息 | `--font-xs` | max(12px, round(U × 12 / 14)) | 12px |
| 代码 | `--appearance-code-size` | 独立偏好 | 11px |

派生角色取整到整数像素。保存的 UI 字号可以是半像素档位，但派生出的小数字号会让行框和 1px 边线落在设备像素之间。

常规文字用 400，紧凑菜单选项用 500，必要的标题与强调选择 500 或 600；700 留给少量强强调。基础行高角色为 UI 1.35、正文 1.55、辅助信息 1.6；具体阅读面可有自己的语义行高，不代表所有组件强制使用同一行高。对话阅读与过程记录的行高是由消息字号派生的整数像素长度（分别为 1.8 与 1.6 倍，取整到 2px），相邻行与卡片因此共享边线。消息内标题依次为正文的 1.5、1.25、1.1 倍并取整，层级不只依赖字重。保留字体自然字距，不缩字或压字距来补救布局。
...
宽度不超过 560px 时，页面、面板边缘和面板内缩为 16px，卡片为 12px（仍受密度影响）。表单和菜单行的共享高度下限为 32px，粗指针下为 44px，并向上取整到 4px 网格。字号或内容增长时，实际高度应继续增长，不能用固定高度裁字。以上是共享字段与菜单行的尺寸角色，并非所有图标按钮的统一尺寸。

### 对齐轴

看起来对齐的边，必须来自同一个共享角色，而不是几个分别调过、恰好接近的数值。

- **面板边缘。** 侧栏行、标题栏、标签栏等界面框架行中，首尾可见字形距离面板边缘 `--pane-inset`。对齐的是字形，不是按钮盒：按钮盒大于字形的控件要减去自己的光学偏移，例如工具栏按钮使用 `--control-toolbar-glyph-offset`。
- **阅读列。** 无框内容（段落、标题、列表、过程记录、回答操作）共用阅读列的起始边。消息表格没有左右边框，首尾列文字也从这条边开始。
- **带框内容。** 代码块、输入框、消息气泡和卡片中，文字与图标字形距离外边缘 `--card-padding`；边框计入这段距离。因此代码块的语言标签、代码与输入框文字共用同一条轴。
- **附属操作列。** 侧栏尾部大小不同的附属操作共用一条中心轴。分组标题无论是否带操作，都预留操作的占位，分组间距不因哪些标题有「+」而变化。
