...
Wuu 保存原生会话引用，并在后续轮次加载。如果 ACP Agent 不支持加载会话，或原会话加载失败，本轮会报错，不会静默创建新历史。只有原生提示请求的响应才能结束一轮，不以一段时间没有输出作为完成依据。停止操作会取消原生任务并清理子进程。OpenCode 使用带密码的本机回环服务，每个进程生成新密码，不作为远程服务开放。
