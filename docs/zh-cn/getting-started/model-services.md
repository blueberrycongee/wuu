...
在**设置 → 模型服务 → Auto** 中，选择判断模型，以及简单、中等、复杂任务对应的执行模型。Auto 位于已连接服务下方。打开开关后显示模型选项，并自动滚动到选项区域；关闭后隐藏选项并保留配置。每行按用途、模型、推理参数排列。模型与服务一起选择，允许跨服务组合。修改后自动保存；保存失败时恢复原设置并显示错误。启用后，在 Wuu 会话的模型菜单选择 **Auto**。
