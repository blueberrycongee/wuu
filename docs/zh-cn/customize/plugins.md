...
查找和发送限定在当前工作区，不包含私有、已归档或其他工作区的会话。同一工作区中的本地分叉
仍可联系；暂不支持跨工作区投递。
...
回信进入队列不等于已经送达，接收会话开始执行后才确认投递。Peers 启用期间，会在重启后使用
原请求标识和保留的结果恢复从主机待执行队列中丢失的回信，但尊重用户主动取消队列的操作。
发送结果尚未确认时取消调用，已经接受请求的目标仍可回传结果。可重试的投递失败不会把已经
完成的结果替换成请求超时消息。
