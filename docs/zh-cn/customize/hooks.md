...
Matcher 不支持 glob 表达式。没有工具名的事件需要空 matcher 或 `*`。匹配的 Hook 按配置顺序运行，遇到第一个错误就停止。在 `PreToolUse` 中，每次替换参数后，后续 Hook 收到的 `tool_input` 都是替换后的值。其他单值字段采用最后提供的值。补充上下文按顺序累积，以空行分隔；后续 Hook 失败不会抹掉先前的上下文。

匹配 `PreToolUse` Hook 的调用会在模型输出完成后串行执行：原始参数无法证明改写后的操作仍适合提前或并发执行。未匹配的调用保留正常调度方式。工具授权仍会检查改写后的参数。
...
| `PostToolUseFailure` | Go 执行错误或富结果标记为 `is_error` 时触发，Hook 错误不替换工具原结果 |
...
退出码 2 始终阻止，即使 stdout 包含 `{}` 或允许继续的决定。阻止原因优先使用 JSON 的 `reason`，否则使用 stderr。退出码 0 使用 JSON 决定，没有决定时继续；其他非零退出码表示执行失败。无效或混杂的 stdout 不会被当作结构化输出。
