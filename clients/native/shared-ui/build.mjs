...
// API 28 ships Chromium 69; polyfill APIs still referenced after chrome69 syntax downlevel.
const polyfill = "Object.hasOwn||(Object.hasOwn=function(o,p){return Object.prototype.hasOwnProperty.call(o,p)});Object.fromEntries||(Object.fromEntries=function(e){var o={};for(var i of e)o[i[0]]=i[1];return o});";
const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data:; connect-src 'none'; base-uri 'none'; form-action 'none'"><style>html,body,#root{margin:0;background:transparent}</style><style>${style}</style></head><body><div id="root"></div><script>${polyfill}${script}</script></body></html>\n`;
const output = path.join(directory, "NativeUI/mascot.html");
const processResult = await build({
  absWorkingDir: path.resolve(directory, "../../.."), entryPoints: [path.join(directory, "src/process.ts")],
  bundle: true, write: false, metafile: true, minify: true, format: "iife", globalName: "WuuProcess", target: "safari17",
  nodePaths: [path.join(desktop, "node_modules")], legalComments: "inline",
  define: { "process.env.NODE_ENV": '"production"', "import.meta.env.DEV": "false", "import.meta.env.MODE": '"production"' },
});
const processScript = processResult.outputFiles[0].text;
const processOutput = path.join(directory, "NativeUI/process.js");
const repository = path.resolve(directory, "../../..");
const sources = [...Object.keys(result.metafile.inputs), ...Object.keys(processResult.metafile.inputs)]
  .filter(file => !file.includes("node_modules/"));
sources.push("clients/native/shared-ui/build.mjs");
...
hashes.push(`${createHash("sha256").update(processScript).digest("hex")}  clients/native/shared-ui/NativeUI/process.js`);
const manifest = hashes.join("\n") + "\n";
const manifestPath = path.join(directory, "NativeUI/sources.sha256");
if (process.argv.includes("--check")) {
  if (await readFile(output, "utf8") !== html || await readFile(processOutput, "utf8") !== processScript || await readFile(manifestPath, "utf8") !== manifest) throw new Error("Native mascot bundle is stale. Run node clients/native/shared-ui/build.mjs");
} else { await mkdir(path.dirname(output), { recursive: true }); await writeFile(output, html); await writeFile(processOutput, processScript); await writeFile(manifestPath, manifest); }
