...
进入电脑后可在底部切换“会话”和“协作”，默认仍打开会话。两种模式共用加密连接，独立保留当前对话、草稿和阅读位置；切换页面不停止电脑任务。协作支持查看房间、与已有 Agent 私聊、创建群聊、收发文字和发送图片/PDF，成员与已加载任务在详情页查看。Agent 配置、创建申请处理和收到的协作附件预览仍需在电脑完成，手机会显示相应入口限制。
...
此协作实现要求电脑支持 `channel/message/list` 的 `latest`、`before_seq` 和 `attachment_metadata_only` 参数。`latest` 选取最新窗口但仍按序号升序返回，`before_seq` 是排除边界；默认调用的旧语义保持不变。手机只请求附件元信息，避免房间历史中的内嵌附件超过远程帧限制；升级手机时也应更新电脑端。
