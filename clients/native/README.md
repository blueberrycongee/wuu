...
iOS 使用 Swift 与 SwiftUI；Android 使用 Kotlin 与 Jetpack Compose。聊天、导航、键盘和附件操作使用原生界面，遵循同一远程协议和恢复语义。iOS 会话的 Agent 小球直接打包电脑端的 React/SVG 组件，在无网络、无原生权限桥接的小型 WebView 中显示；动画状态、形状和配饰不在手机重复实现。旧 `clients/mobile`、`clients/mobile-web`、`clients/mobile-app` 手机实现已经停止开发，保留作交互与历史参考。

账号服务器保存用户明确开启同步后的对话文字副本。手机先读本地缓存，再按服务器 generation、cursor 和 revision 补齐变化；无变化的服务器历史不会重复写入磁盘缓存。电脑离线时仍可读已同步历史；发送、停止与新建会话需要在线电脑。历史读取使用账号 HTTPS API，执行控制使用现有端到端加密远程通道，不把账号服务器变成 Agent 执行器。

登录保留服务器选择、GitHub 主入口和更多登录方式；进入电脑后保留侧滑会话列表、消息页顶部操作和系统返回顺序。账号令牌与设备身份使用系统安全存储，退出或凭据被撤销时清除账号会话及历史缓存。

## 当前能力

App 重启后恢复上次的电脑、工作区和会话，恢复位置仅对原登录及仍有权限的电脑有效。账号设置可在会话列表中打开。
...
iOS 会话将连续工具调用聚合为动作摘要，直接复用桌面端的分类、摘要和小球动效，不提供明细展开；运行时可在输入栏点击停止图标。Android 仍可展开工具参数、文字结果与错误，并预览工具结果中的图片。主机发送有大小限制的开始和完成记录，不发送工具输出增量。工具详情不进入账号服务器历史或对话文字导出；完整工具过程和扩展自定义界面可在电脑查看。

iOS 的摘要规则在一个 JavaScriptCore 实例中执行，文字由 SwiftUI 排版，历史工具组不创建 WebView。工具生成的图片保留在摘要下方。消息投影按更新缓存，Markdown 解析离开主线程；列表从滚动容器读取实际位置，手动上翻不被新消息拉回底部。
...
iOS 将预览解码为最长边 288 像素，按固定 96×72 点在文字气泡外排列；加载完成不改变行高，同一图片的并发请求会合并，离屏请求可取消。
...
iOS 安装包使用仓库中已提交的小球与过程摘要资源快照，普通原生构建不要求安装 Node.js，也不要求快照跟随电脑端源码同步。需要主动采用电脑端表现层更新时，在安装好 `desktop` 依赖的仓库中运行：
...
iOS Debug 构建可通过 `xcrun simctl launch booted ai.wuu.native --native-ui-fixture` 打开本地长会话样本：80 轮消息、Markdown、图片网格、纯图片消息和工具图片，不读取账号或连接电脑。检查正常与辅助功能大字号、明暗主题、快速滚动、图片预览，以及会话上翻后点击“末尾追加”是否保留阅读位置。正常启动恢复原入口；Release 不包含这些验收入口。样本只验证渲染，不替代真实协议集成或真机性能测试。

## 验收边界

自动验证与模拟器视觉检查分别记录，不将编译或协议测试视为真机验收。发布前需要使用本次代码重新运行完整验证，并检查真机滚动帧率、键盘、VoiceOver/TalkBack、主题和屏幕尺寸。iOS 会话小球的 WebView 首次加载与内存表现也需覆盖真机。
