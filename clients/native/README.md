...
iOS 首次读取最新窗口，后续按序号读取新增消息并保留一条重叠记录；任务与创建申请单独轮流刷新。在房间内阅读时降低整个目录的刷新频率，避免重复已读写入。无变化的服务器历史不会重复写入磁盘缓存。
...
iOS Debug 构建可通过 `xcrun simctl launch booted ai.wuu.native --native-ui-fixture` 打开本地长会话样本：80 轮消息、Markdown、图片网格、纯图片消息和工具图片，不读取账号或连接电脑。使用 `--native-collaboration-fixture` 检查同一批内容的协作布局。检查正常与辅助功能大字号、明暗主题、快速滚动、图片预览，以及会话上翻后点击“末尾追加”是否保留阅读位置。正常启动恢复原入口；Release 不包含这些验收入口。样本只验证渲染，不替代真实协议集成或真机性能测试。
