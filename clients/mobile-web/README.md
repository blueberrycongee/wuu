...
先区分首次打开、已有页面回前台、浏览器回收页面后的重新加载。页面回前台会探测连接；续传成功时保留工作台，只有需要新建远程连接时才重新同步状态。浏览器回收页面后仍需加载代码、建立连接和读取会话，不能用暖恢复的耗时标准验收。

前台探测默认最多等 5 秒，稳态心跳仍为 30 秒；这个期限不包括后续建连和状态同步。重复前台事件不会延后在途探测期限。极低带宽下，大消息可能阻塞 pong，因此应同时验证传输历史期间回前台的行为，留意误判断连。新连接同步后的首次工作区读取复用刚取得的状态，之后刷新仍请求宿主。

使用 Tailscale 时，让手机保持 5G、Tailscale 在线并打开页面，在电脑执行以下只读检查。输出可能包含设备名和网络地址，分享前需脱敏；不要分享配对链接、凭据或完整浏览器存储。

```bash
tailscale status
tailscale ping --c 10 --timeout 3s <手机的 Tailscale IP>
tailscale netcheck
tailscale serve status
```

`ping` 持续显示 `via DERP` 才能证明本次探测使用中转；设备记录中的 `Relay` 字段单独不足以判断。`netcheck` 的区域延迟是电脑到中转节点的测量，不能代替手机端到端延迟。电脑 UDP 可用也不代表手机侧或两端 NAT 允许直连。参照 [Tailscale 连接类型](https://tailscale.com/docs/reference/connection-types) 排查两端 UDP、防火墙和 NAT；变更一项后重新测量，不用开放公网 Funnel 或关闭防火墙来尝试提速。Serve 的 HTTPS 反向代理负责网页入口，DERP 负责隧道传输，修改 Serve 地址不会自动改善中转路径。

用浏览器网络面板分段记录 HTML、JS/CSS、WebSocket 建连、工作区同步和可见会话恢复的耗时，同时注明版本、是否命中缓存、网络类型和 `ping` 路径。只导出脱敏的时间与大小，不分享包含认证头、配对信息或会话内容的 HAR。分别复测首次打开、普通刷新、短暂后台、切换 Wi-Fi/5G、断网后恢复和宿主重启；检查草稿保留、消息不重复、离线操作明确失败以及恢复失败可重试。

内置网页服务对带内容指纹的构建资源使用长期缓存，HTML 重新验证。部署时应保留旧版本资源，至少覆盖仍在打开的页面可能请求的延迟加载模块；反向代理不能把 HTML 改成长期缓存，也不能删除 `Vary: Accept-Encoding`。限速回归见下方端到端验证；它不能替代真机上的高延迟、网络切换和浏览器页面回收测试。
...
Web 的切分支、新建分支、提交、生成提交信息和创建 PR 通过固定的 `desktop/git/*`
宿主接口复用桌面 GitService，沿用同一工作区校验、任务运行检查及已有确认界面。
创建 PR 使用配对电脑上的 GitHub CLI 和登录状态。
...
Web 在 `workspace/list` 上协商 `remote_delivery: 1`。桌面 bridge 在 `thread/resume`
请求上启用 `history_page`，Go 在发送快照前选择近期历史，避免完整旧历史经过 Electron。
每页最多 20 轮、目标 256 KiB，允许按 item 边界拆分长轮次；大于 64 KiB 的单项内容
保留预览，通过“加载完整内容”按需取回。完整内容分块传输，不会静默丢弃。
旧页通过 `remote/history/read` 转发至 `thread/history/read`。游标定位 turn/item，
追加消息不移动边界，删除边界后明确报错。浏览器合并同轮次旧 items 时保留更新的实时内容。
冷会话仍需要从持久化历史重建 runtime；本优化不等于数据库已经按页读取。
`response_only` 避免重复广播，浏览器在收到响应后本地同步状态。

大于 16 KiB 的会话图片携带包含来源位置及内容摘要的 `remote_ref`。读取时回到该
会话的原始内容，图片可用性不再依赖远程内存缓存；源内容删除或变更后明确失败。
视口附近自动读取最长边 384px 的 JPEG 缩略图，最多两张并发；点击后才获取原图，
每块最多 128 KiB 编码字符，最多四块并发。缩略图解码前校验尺寸，不能生成时保留
原图加载按钮。原图缓存按 16 MiB 编码字符限制，超出预算的单图不驻留 bridge 缓存。
编辑重发引用图片时，桌面先取回原始字节。这些操作全部使用既有密封通道，
没有公开的会话图片地址。尚未归属会话 item 的临时附件仍使用有界临时缓存。
