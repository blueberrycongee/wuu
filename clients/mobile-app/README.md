...
> **已终止开发，不再维护（2026-09-12）。** 此目录保留 Capacitor 手机客户端的历史代码和构建记录，供 UI、登录结构和导航动线参考。后续手机功能与修复转向 [原生客户端](../native/README.md)。下文描述旧实现，不代表新的手机技术路线或支持承诺。

iOS 和 Android 使用打包在 App 内的共享 Wuu 界面，连接用户自己部署的账号服务。Agent、终端、文件和工作区都在所选电脑上运行。手机不代替电脑执行任务，也不会唤醒离线电脑。

旧版功能对应与手机交互见 [历史功能清单](FEATURES.md)。这些构建和测试方法不代表当前原生客户端已通过发布验收；当前状态见 [原生客户端说明](../native/README.md)。
...
账号退出及检测到撤销会清除手机保存的 Wuu 登录状态和账号相关界面偏好，保留语言选择与按服务端、账号隔离的设备私钥，让重新登录复用同一设备记录。清除应用存储或更换设备仍可能产生新记录；历史重复记录需手动移除，不能仅按名称合并。主动退出在离线时也会清除本地登录，并提示远端撤销未确认；刷新时的临时网络错误保留凭据以便恢复。

账号电脑的「对话历史」支持主动开启服务端可读的文字同步，先显示手机 IndexedDB 缓存，再向账号服务补齐变化。电脑离线时可阅读已上传的历史；最近正文最多缓存二十条、约 16 MiB。退出账号会清除历史缓存，关闭同步会删除该电脑的服务器副本；离线设备下次联网才会获知远端撤销。完整历史和工作区仍以电脑为准，文字副本不能恢复可执行会话。发送消息和运行任务需要点击「连接电脑」。更新 App 前保留电脑数据；恢复码与电脑数据备份说明见服务端文档。
