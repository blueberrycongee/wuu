...
## 2026-09-11 Android 后台同步

本次工作区实现增加 Android `remoteMessaging` 前台服务，并以连接为单位持有和释放服务。只有原生确认服务启动后才保留后台传输；启动失败或服务丢失时降级为暂停连接。恢复连接用 `workspace/list` 回复作为消息回放完成的屏障，随后才显示已连接。

验证使用 API 36 Android 模拟器、实际 WebView、加密 relay 和共享桌面 host，模型输出由确定性测试服务生成。后台阶段断开 CDP 调试连接。直连网络测试包信任本机测试 CA，交付 APK 使用正常 TLS 配置。

| 场景 | 结果 |
| --- | --- |
| 连续 10 次前后台切换 | 460 段全部有序收到，0 次重连，每次唤醒时内容已追平 |
| 40 KiB/s 弱网，后台 60 秒 | 1,240 段全部有序收到，0 次重连，唤醒时已追平 |
| 50 段/秒、每段 400 字节，后台 120 秒 | 6,100 段、2.44 MB 内容全部有序收到，0 次重连，唤醒时已追平 |
| 强制 Doze 45 秒 | 480 段全部有序收到，0 次重连 |
| 完整聊天界面，弱网后台 45 秒 | 470 段全部收到；返回时数据已追平，最新内容的 DOM 更新仍需约 1.8 秒 |
| 主动暂停传输 60 秒后弱网回放 | 1,240 段完整有序；最后一段在唤醒后 47.7 秒到达，48.1 秒才显示已连接，未提前就绪 |
| 空闲后台 60.86 秒 | 加密连接上行 730 字节、下行 564 字节，含返回时探测；不是整台手机的流量统计 |

相关 TypeScript 测试 43 项通过；完整 mobile-web 测试 93 项通过、1 项跳过，类型检查和 Web/Android 构建通过。原生测试覆盖启动后立即取消、服务意外停止、重叠连接持有及最后释放。测试发现的前台服务启动取消崩溃已修复；另一项并行登录测试重新安装 APK 导致的中断不计入结果，已换隔离模拟器重跑。

本次 Debug APK：`/tmp/wuu-background-implementation/wuu-android-background-sync.apk`，SHA-256 为 `2baa91fbb7d80672e6938549d9d92ee6ec0669be3864a33ca9aeaee55d20b560`。原始结果、原生日志、界面截图与可重跑脚本见 `/tmp/wuu-background-implementation/README.md`；临时目录可能被系统清理。

尚未验证用户真机、厂商省电策略、整夜待机及耗电。前台服务不能保证进程永不被终止，也不能保证网络始终可用；长文本返回后的渲染耗时仍是明确的体验限制。
