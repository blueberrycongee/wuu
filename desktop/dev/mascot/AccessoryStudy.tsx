...
const LABELS = { none: "本体", beanie: "软帽", "hard-hat": "安全帽", headset: "耳麦", leaf: "小叶子" };
...
export function AccessoryStudy(): JSX.Element {
  ...
  const identityTraits = { ...WUU_MASCOT_TRAITS, shape: AGENT_AVATAR_SHAPES[shapeIndex].trait };
  ...
  return <main className="beanie-study" data-theme={dark ? "dark" : "light"}>
    ...
    <section className="accessory-collection" aria-label="选择配饰">
      {WUU_MASCOT_ACCESSORIES.map(item => <button type="button" key={item} aria-pressed={accessory === item} aria-label={LABELS[item]} onClick={() => setAccessory(item)}>
        <WuuMascot size={72} identityTraits={identityTraits} accessory={item} identityHue={hue} activity={activity} style={camera} />
        <span>{LABELS[item]}</span>
      </button>)}
    </section>
    <section className="beanie-hero" aria-label="配饰与本体对照">
      <figure>
        <WuuMascot size={200} identityTraits={identityTraits} accessory="none" identityHue={hue} activity={activity} followPointer={follow} style={camera} />
        <figcaption>本体</figcaption>
      </figure>
      <figure>
        <WuuMascot size={200} identityTraits={identityTraits} accessory={accessory} identityHue={hue} activity={activity} followPointer={follow} style={camera} />
        <figcaption>{LABELS[accessory]}</figcaption>
      </figure>
    </section>
    <div className="beanie-controls">
      <label>形状<select aria-label="形状" value={shapeIndex} onChange={e => setShapeIndex(Number(e.target.value))}>
        {AGENT_AVATAR_SHAPES.map((shape, i) => <option key={shape.id} value={i}>{["圆形", "圆角方形", "胶囊形", "三角形", "菱形"][i]}</option>)}
      </select></label>
      ...
      <label>动作<select aria-label="动作" value={activity} onChange={e => setActivity(e.target.value as WuuMascotActivity)}>
    ...
    <section className="beanie-sizes" aria-label="实际尺寸">
      {[28, 40, 64, 96].map(size => <figure key={size}>
        <div><WuuMascot size={size} identityTraits={identityTraits} accessory={accessory} identityHue={hue} activity={activity} style={camera} /></div>
        <figcaption>{size} px</figcaption>
      </figure>)}
    </section>
    <section className="beanie-colors" aria-label="不同颜色">
      {[14, 52, 96, 150, 202, 222, 288, 350].map(color => <WuuMascot key={color} size={64} identityTraits={identityTraits} accessory={accessory} identityHue={color} activity={activity} style={camera} />)}
    </section>
    <section className="shapes" aria-label="五种形状">
    ...
    <section className="shapes" aria-label="形状与动作">
