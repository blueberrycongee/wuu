...
import { ChannelSessionInspector } from "../../src/renderer/ChannelSessionInspector";
import { useSessionInspectorWindow } from "../../src/renderer/SessionInspectorWindow";
...
host.wuu = {
  setSessionInspectorExpansion: host.inspectorNative,
  ...
  readChannelSession: async ({ sessionRef }: { sessionRef: string }) => {
    ...
    return {
      ...
      thread: { id: sessionRef, turns: Array.from({length: 24}, (_, index) => ({ id: `turn-${index}`, status: "completed", items: [
...
function Preview() {
  ...
  const inspector = useSessionInspectorWindow(shell);
  const extension = inspector.extension;
  const panel = extension?.target ? <ChannelSessionInspector key={extension.target.sessionRef} sessionRef={extension.target.sessionRef}
    name={extension.target.name} left={extension.baseWidth} width={extension.panelWidth} onClose={() => void inspector.close()} /> : null;
  if (!new URLSearchParams(location.search).has("rail")) return <><div ref={shell} className={extension ? "session-inspector-expanded" : ""}
    style={{ height: "100vh", display: "flex", width: extension?.baseWidth, "--inspector-original-gutter": extension?.gutter } as React.CSSProperties}>
    <ChannelView selectedRoomID="room" onInspectSession={target => { void inspector.open(target); }} /></div>{panel}</>;
