...
import { agentAvatarConfig, parseAgentAvatarConfig, serializeAgentAvatarConfig } from "./AgentAvatarMark";

describe("saved avatar accessories", () => {
  it("discards retired artwork without losing a saved shape or colour", () => {
    const config = agentAvatarConfig("mascot-v1:cloud:crown:202");
    expect(config).toEqual({ shape: "cloud", hue: 202, accessory: "none" });
    expect(parseAgentAvatarConfig(serializeAgentAvatarConfig({ ...config, accessory: "hard-hat" })))
      .toEqual({ shape: "cloud", hue: 202, accessory: "hard-hat" });
  });
