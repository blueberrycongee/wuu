...
工具执行和完整会话留在电脑。账号用户可在手机「对话历史 → 同步设置」主动开启文字副本，开启后 PostgreSQL 保存标题、对话显示文字和运行状态，**服务端运营方可读取这些副本**；不会自动上传工具输出、附件或工作区文件。关闭会删除在线数据库中的副本，数据库备份需要另行管理。默认关闭同步，独立 QR 配对不使用此功能。

实时中继仍转发通过认证的 X25519 临时握手和 AES-256-GCM 加密帧，不保存这些帧或附件。每次连接重新派生方向密钥，计数器拒绝重放。账号模式通过服务端目录分发设备身份，因此**恶意或被入侵的账号服务端可以授权攻击者设备**；此模式不能承诺抵御恶意身份服务器。原有 QR 配对的独立公钥固定信任流程仍可单独使用。
...
账号数据库备份包含已开启同步的会话文字副本，可恢复服务器历史读取；它不包含恢复 Agent 执行所需的完整会话与工作区。分别备份每台电脑的 `WUU_HOME` 和工作目录，恢复文件权限；不要将同一个 `remote.json` 同时复制到两台运行中的电脑，它们会共享设备身份并互相顶下线。
